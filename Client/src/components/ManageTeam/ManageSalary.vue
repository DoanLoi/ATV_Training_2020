      ...
      <div class="edit-form" @click="onBlurSearch()">
        ...
        <div class="result-search" :style="{ display: showResult }">
          ...
          <ul style="margin-left:-40px" v-else-if="resultSearchUser.length > 0">
            <li v-for="user in resultSearchUser" :key='user.id' @click="[keyword=user.name,internToAdd=user.internid]">
              <img
  ...
  methods: {
    ...
    async onAddSalary(){
       if(this.internToAdd ==''){
        ...
        let res = await this.addSalaryForIntern({internid:this.internToAdd,salary:this.salary,teamid:this.teamid});
        if (res == true) {
